...
(e) With $\sigma _{i} \mapsto U_{i,i+1}$, the relation \eqref{eq:definingRelationOfBraidGroupOnSphere} can be written as:
\begin{equation*}
...
[(Unsolved! This isn't over. I will be back. ] From my perspective, we can only focus on the $ \gamma^{\text{FIVE}}$ term.

(f) Such additional phase factor $e^{i\phi} $ could be added to the right hand side of equ.~\ref{FIVE_phase}, we can obtain that $\alpha= \frac{\pi}{8}\frac{2N+ 4\phi /\pi}{2N-1}$, thus $\phi=-\pi/4$ for even parity, $\phi=-\pi/2$ for odd parity, finally we get $\alpha=\pi/8$.
